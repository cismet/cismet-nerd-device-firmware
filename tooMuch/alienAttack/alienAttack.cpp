...
void alienAttackLoop()
{
  ...
  if (fase == 2)
  {
    ...
    tft.drawString("aascore: " + String(aascore), 25, 120, 2);
...
void collision()
{
  for (int i = 0; i < 10; i++)
  {
    for (int j = 0; j < 8; j++)
    {
      ...
      {
        aascore++;
        tft.drawString(String(aascore), 48, 1, 2);
        by[i] = 250;
        el[j] = el[j] - 1;
        if (el[j] == 0)
        {
