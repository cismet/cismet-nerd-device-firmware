...
void breakoutLoop()
{
  ...
  if (fase == 0)
  {
    ...
    if (digitalRead(0) == 0 || digitalRead(35) == 0)
    {
      if (breakout_pom == 0)
      {
        ...
        tft.setCursor(0, 0, 2);
        tft.println("score " + String(bascore));
  ...
  if (fase == 1)
  {
    ...
    if (bascore == 16 || bascore == 33 || bascore == 50 || bascore == 67 || bascore == 84 || bascore == 101 || bascore == 118 || bascore == 135 || bascore == 152 || bascore == 169)
      newLevel();
  ...
  if (fase == 2)
  {
    ...
    delay(1000);
    tft.drawString("2 x Btn for restart", 10, 220, 2);
  ...
  if (fase == 3)
  {
    ...
    if (digitalRead(leftButton) == LOW || digitalRead(rightButton) == LOW)
    {
      ...
      pxn = px;
      xs = amount[random(4)] * vrije[random(2)];
      fase = 0; // Return to initial phase
    }
    delay(100);
  }
}
